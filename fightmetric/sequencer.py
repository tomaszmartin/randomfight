...
class Sequencer:
    ...
    def impute_attendance(self):
        ...
        filled["year"] = filled["date"].apply(
            lambda x: datetime.datetime.strptime(x, "%Y-%m-%d").year
        )
        grouped = filled.groupby(["year"]).mean()
        self.attendance = grouped.drop(
            ["height", "position", "reach", "round", "time", "titlefight", "weight"],
            axis=1,
        )

    def fill_attendance(self):
        """Attendence is imputed as the average for the current year."""
        self.data["year"] = self.data["date"].apply(
            lambda x: datetime.datetime.strptime(x, "%Y-%m-%d").year
        )
        avg_attendance = self.data["year"].apply(
            lambda x: self.attendance.ix[int(x)]["attendance"]
        )
    ...
    def age(born_at, fight_at):
        ...
        return (
            fight_at.year
            - born_at.year
            - ((fight_at.month, fight_at.day) < (born_at.month, born_at.day))
        )
    ...
    def build_fighter_stats(self, list_of_fights):
        ...
        for i, current_fight in enumerate(list_of_fights):
            ...
            if i == 0:
                ...
                stats[i]["fighter"]["current"] = {
                    "attendance": current_fight["attendance"],
                    "position": current_fight["position"],
                    "rounds": current_fight["time format"],
                }
                stats[i]["fighter"]["history"] = {
                    "attendance": 0.0,
                    "position": 0,
                    "time": 0.0,
                    "wins": 0,
            ...
            if i > 0:
                ...
                stats[i]["fighter"]["history"]["attendance"] += previous_fight[
                    "attendance"
                ]
            ...
            # All stats
            stats[i]["fighter"]["age"] = self.age(
                current_fight["birth"], current_fight["date"]
            )
        ...
        for i, fight in enumerate(fights):
            ...
            if i == 0:
                ...
                for key in fight["fighter"]["cumulative"][result].keys():
                    for subkey, value in fight["fighter"]["cumulative"][result][
                        key
                    ].items():
                        fight["fighter"]["cumulative"][result][key][subkey] += previous[
                            "fighter"
                        ]["history"][key][subkey]
                stats.append(fight)

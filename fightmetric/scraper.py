...
def stat_to_dict(data):
    ...
    for key, value in data.items():
        ...
        if key == "knockdowns":
            landed = 0.0
            if (data["method"] == "KO" or data["method"] == "KO/TKO") and data[
                "result"
            ] == "Win":
                landed = 1.0
...
def exchange_stats(fight):
    ...
    for key in fighter["stats"].keys():
        ...
        fighter["stats"][key]["avoided"] = (
            opponent["stats"][key]["thrown"] - opponent["stats"][key]["landed"]
        )
        opponent["stats"][key]["received"] = fighter["stats"][key]["landed"]
        opponent["stats"][key]["avoided"] = (
            fighter["stats"][key]["thrown"] - fighter["stats"][key]["landed"]
        )
...
def extract_title(soup):
    ...
    return get_name(
        soup.find("span", {"class": "b-content__title-highlight"}).get_text().strip()
    )
...
def extract_attrs(soup):
    ...
    result = soup.find_all(
        "i",
        {"class": ["b-fight-details__text-item_first", "b-fight-details__text-item"]},
    )
...
def clean_data(data):
    ...
    for key, value in data.items():
        ...
        if key in ["rev.", "td %", "sig. str. %", "pass", "details", "sig. str."]:
            del cleaned[key]
        if key == "dob":
            del cleaned[key]
            cleaned["birth"] = value
...
def scrape_links(content):
    ...
    fights = [
        link["data-link"]
        for link in soup.find_all("tr", {"class": "js-fight-details-click"})
    ]
    fighters = [
        link["href"] for link in soup.find_all("a", href=re.compile("fighter-detail"))
    ]
...
def scrape_fight_data(content, link, position):
    ...
    for table in soup.find_all("table", {"class": ""}):
        ...
        for name, value_pair in zip(names, value_pairs):
            value_pair = [
                clean_value(value) for value in re.split(r"\n\n+", value_pair)
            ]
            value_pair = [
                int(value) if value.isdigit() else value for value in value_pair
            ]

...
class Sequencer(object):
    ...
    def enrich(raw):
        ...
        opponent.rename(columns={'fighter': 'opponent', 'opponent': 'fighter'}, inplace=True)
    ...
    def create_stat(self, current_stat=None, previous_stat=None, current_fight=None, previous_fight=None):
        ...
        # Add stats
        current_stat['fighter']['history']['since_last_fight'] = (current_stat['date'] - previous_stat['date']).days
        ...
        current_stat['fighter']['history']['positions'] += float(previous_fight['position'])
    ...
    def build_stats(self, fights):
        ...
        for i, current_fight in enumerate(fights, 0):
            if i == 0:
                ...
                current_stat = self.create_stat(current_stat=current_stat,
    ...
    def exchange(data):
        ...
        for i, idx in enumerate(ids):
            if i % 1000 == 0:
                print('Exchanging data {} from {} in {:.2f}'.format(i, len(ids), time.time() - start))
                start = time.time()
            pair = []
            for current in data:
                if current['id'] == idx:
...
class Cumulator(Sequencer):
            ...
            if i == 0:
                ...
                for result in ['win', 'loss']:
                    for method in fight['opponent']['history'][result].keys():
                        fight['fighter']['cumulative'][placement][result][method] += previous_stat['opponent']['history'][result][method]
                stats.append(fight)
    ...
    def transform(self):
        ...
        for i, fighter in enumerate(fighters):
            if i % 1000 == 0:
                print('Transformed {} out of {} stats for {:.2f} seconds'.format(i, num_of_fighters, time.time() - start))
                start = time.time()
            fights = self.get_fights_for_fighter(fighter)
            current = self.build_stats(fights)

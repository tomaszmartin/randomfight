...
class Sequencer:
        ...
        for i, current_fight in enumerate(fights):
            try:
                ...
                if i > 0:
                    ...
                    # Add stats
                    current["fighter"]["history"]["since_last_fight"] = (
                        current["date"] - stats[i - 1]["date"]
                    ).days
                    current["fighter"]["history"]["fights"] += 1.0
                    current["fighter"]["history"]["time"] += float(
                        previous_fight["time"]
                    ) * float(previous_fight["position"])
                    current["fighter"]["history"]["positions"] += float(
    ...
    def exchange(self, data):
        ...
        for i, current in enumerate(ids):
            if i % 1000 == 0:
                print(
                    "Exchenging data {} from {} in {:.2f}".format(
